...
@property NSMutableArray<Task *> *tasksProgress;
...
- (void)viewDidLoad {
    ...
    _tasksProgress = [NSMutableArray new];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"status == %hd", 1];
    _tasksProgress = [[_tasks filteredArrayUsingPredicate:predicate] mutableCopy];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if(_isFiltered){
        NSPredicate *low = [NSPredicate predicateWithFormat:@"status == %hd && priority == %hd", 1, 0];
        _tasksLow = [[_tasks filteredArrayUsingPredicate:low] mutableCopy];
        NSLog(@"_tasksLow = %lu",(unsigned long)_tasksLow.count);
        NSPredicate *med = [NSPredicate predicateWithFormat:@"status == %hd && priority == %hd", 1, 1];
        _tasksMed = [[_tasks filteredArrayUsingPredicate:med] mutableCopy];
        NSLog(@"_tasksMed = %lu",(unsigned long)_tasksMed.count);
        NSPredicate *high = [NSPredicate predicateWithFormat:@"status == %hd && priority == %hd", 1, 2];
    ...
    if(_isFiltered){
        ...
        numberOfRows = _tasksProgress.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(_isFiltered){
        ...
        task = _tasksProgress[indexPath.row];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(_isFiltered){
        ...
        task = [_tasksProgress objectAtIndex:indexPath.row];
    }
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self showAlert:@"Delete" message:@"Do you want to delete this task?" action: [UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            if(self->_isFiltered){
                ...
                [self->_tasksProgress removeObjectIdenticalTo:task];
            }else{
                task = [self->_tasksProgress objectAtIndex:indexPath.row];
                [self->_tasksProgress removeObjectAtIndex:indexPath.row];
            }
...
-(void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length > 0) {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"title CONTAINS[cd] %@ && status == %hd", searchText, 1];
        _tasksProgress = [[_tasks filteredArrayUsingPredicate:predicate] mutableCopy];
    } else {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"status == %hd", 1];
        _tasksProgress = [[_tasks filteredArrayUsingPredicate:predicate] mutableCopy];
